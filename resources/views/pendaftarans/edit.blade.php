...
@section('content')
    <div class="container mt-5">
        ...
        <h2 class="text-center mb-4">Formulir Edit Pendaftaran Santri</h2>
        ...
        <form action="{{ route('pendaftarans.update', $pendaftaran->id) }}" method="POST" class="needs-validation"
            ...
            <div class="mb-3">
                ...
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" id="status_diterima" name="status" value="diterima"
                        {{ old('status', $pendaftaran->status) == 'diterima' ? 'checked' : '' }} required>
                    <label class="form-check-label" for="status_diterima">Diterima</label>
                </div>

                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" id="status_ditolak" name="status" value="ditolak"
                        {{ old('status', $pendaftaran->status) == 'ditolak' ? 'checked' : '' }} required>
